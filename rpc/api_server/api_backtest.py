...
from configuration.config_validation import validate_config_consistency
from data.btanalysis import get_backtest_resultlist, load_and_merge_backtest_result
from enums import BacktestState
from exceptions import DependencyException
from rpc.api_server.api_schemas import (BacktestHistoryEntry, BacktestRequest,
                                                  BacktestResponse)
from rpc.api_server.deps import get_config, is_webserver_mode
from rpc.api_server.webserver import ApiServer
...
async def api_start_backtest(bt_settings: BacktestRequest, background_tasks: BackgroundTasks,
    ...
    def run_backtest():
        from optimize.optimize_reports import (generate_backtest_stats,
                                                         store_backtest_stats)
        from resolvers import StrategyResolver
...
def api_get_backtest(ws_mode=Depends(is_webserver_mode)):
    ...
    """
    from persistence import LocalTrade
    if ApiServer._bgtask_running:
        return {
            "status": "running",
            "running": True,
