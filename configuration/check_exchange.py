...
from exchange import (available_exchanges, is_exchange_known_ccxt,
...
def check_exchange(config: Dict[str, Any], check_for_bad: bool = True) -> bool:
    ...
    if not valid:
        if check_for_bad:
            raise OperationalException(f'Exchange "{exchange}"  will not work with  '
                                       f'Reason: {reason}')
        else:
            logger.warning(f'Exchange "{exchange}"  will not work with  Reason: {reason}')
