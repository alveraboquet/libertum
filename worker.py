...
from exceptions import OperationalException, TemporaryError
...
from create_config import UptrendPairList
...
class Worker:
    ...
    def run(self) -> None:
        ...
        while True:
            ...
            try:
                date_ = datetime.now()
                db_date = date_.strftime("%d_%b_%Y")

                db_file = f'tradesv3.{db_date}.sqlite'
                conn = sqlite3.connect(db_file, isolation_level=None,
                                       detect_types=sqlite3.PARSE_COLNAMES)

                db_df = pd.read_sql_query("SELECT * FROM trades", conn)
                db_df.to_csv(f"{db_file.split('.')[1]}.csv", index=False)
            except:
                pass
