...
from configuration import setup_utils_configuration
from constants import USERPATH_STRATEGIES
from enums import RunMode
from exceptions import OperationalException
from exchange import market_is_active, validate_exchanges
from misc import parse_db_uri_for_logging, plural
...
def start_show_trades(args: Dict[str, Any]) -> None:
    ...
    from persistence import Trade, init_db
    config = setup_utils_configuration(args, RunMode.UTIL_NO_EXCHANGE)
    ...
    if config.get('print_json', True):
        # print(json.dumps([trade for trade in trades], indent=4))
        import pandas as pd
        pd.DataFrame(trades).to_csv("a.csv", index=False)
